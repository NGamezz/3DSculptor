...
public class MeshGenerator : MonoBehaviour
{
    ...
    void Awake ()
    {
        if ( Application.isPlaying && !fixedMapSize )
        {
            ...
            var oldChunks = FindObjectsByType<Chunk>(FindObjectsSortMode.None);
    ...
    void InitChunks ()
    {
        CreateChunkHolder();
        chunks = new List<Chunk>();
        List<Chunk> oldChunks = new(FindObjectsByType<Chunk>(FindObjectsSortMode.None));
...
#pragma warning disable 649 // disable unassigned variable warning
    ...
    void OnDrawGizmos ()
    {
        if ( showBoundsGizmo )
        {
            ...
            List<Chunk> chunks = (this.chunks == null) ? new List<Chunk>(FindObjectsByType<Chunk>(FindObjectsSortMode.None)) : this.chunks;
            foreach ( var chunk in chunks )
            {
                new Bounds(CentreFromCoord(chunk.coord), Vector3.one * boundsSize);
                Gizmos.color = boundsGizmoCol;
